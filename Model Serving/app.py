...
def load_data():
    ...
    behaviors_data = {
        "dev": load_behaviors('validation/behaviors.tsv'),
        "test": load_behaviors('testing/behaviors.tsv')
    }

    print(behaviors_data["dev"].info())
    news_data = {
        "train": load_news('training/news.tsv'),
        "dev": load_news('validation/news.tsv'),
        "test": load_news('testing/news.tsv')
    }
...
#code for daily update task
def daily_update_task():
    """
    Background task to trigger the daily update process every 24 hours.
    """
    while True:
        try:
            print("Starting daily update process...")
            
            # Reload behaviors and news data
            args =model_train_with_mlflow.parse_args()
            model_train_with_mlflow.main(args)
            print("Data reloaded successfully.")
            
        except Exception as e:
            print(f"Error during daily update: {e}")
        
        # Wait for 24 hours (86400 seconds) before the next update
        time.sleep(86400)

@app.before_request
def initialize_data():
    global model, behaviors_data, news_data, news_features, tokenizer
    model, behaviors_data, news_data, news_features, tokenizer = load_data()
